...
document.addEventListener("DOMContentLoaded", function () {
    const cards = document.querySelectorAll(".faculty-card"); // Sélectionner toutes les cartes

    function handleScroll() {
        const triggerBottom = window.innerHeight * 0.85; // Déclenche l'animation lorsque l'élément atteint 85% de l'écran

        cards.forEach(card => {
            const cardTop = card.getBoundingClientRect().top; // Obtenir la position de la carte dans le viewport

            if (cardTop < triggerBottom) {
                card.classList.add("visible"); // Ajouter la classe "visible" pour activer l'animation
            }
        });
    }

    window.addEventListener("scroll", handleScroll); // Écouter l'événement de défilement
    handleScroll(); // Vérifier au chargement pour les cartes déjà visibles
});
